...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = 'Пожалуйста, выбирете размер и материал картины';
...
var drop = function drop() {
  ...
  ['dragenter', 'dragleave', 'dragover', 'drom'].forEach(function (eventName) {
  ...
  function highlight(item) {
    item.closest('.file_upload').style.border = '5px solid deepskyblue';
    item.closest('.file_upload').style.backgroundColor = 'yellow';
  }
